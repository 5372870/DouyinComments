...
class TestGetWebid(unittest.TestCase):
    ...
    @patch("common.requests.get")
    def test_all_retries_exhausted_fallback_to_cookie(self, mock_get):
        mock_get.side_effect = requests.ConnectionError("always fails")

        cookie_dict = {"s_v_web_id": "verify_1234567890123456789_abc"}
        result = get_webid(self.headers, cookie_dict=cookie_dict)
        self.assertEqual(result, "12345678901234567")
        self.assertEqual(mock_get.call_count, 3)

    @patch("common.requests.get")
    def test_all_retries_exhausted_no_cookie_returns_none(self, mock_get):
        mock_get.side_effect = requests.ConnectionError("always fails")

        result = get_webid(self.headers, cookie_dict={})
        self.assertIsNone(result)

    @patch("common.requests.get")
    def test_200_but_no_match_then_retry(self, mock_get):
        mock_no_match = MagicMock()
        mock_no_match.status_code = 200
        mock_no_match.text = "<html>no match</html>"
        mock_match = MagicMock()
        mock_match.status_code = 200
        mock_match.text = (
            '<script>'
            '\\"user_unique_id\\":\\"7777777777777777777\\"'
            '</script>'
        )
        mock_get.side_effect = [mock_no_match, mock_match]

        result = get_webid(self.headers)
        self.assertEqual(result, "7777777777777777777")
        self.assertEqual(mock_get.call_count, 2)
